...
# --------- Example of the Main Training Process ---------
...
def train_federated_actor_critic(
    ...
    gamma=0.99,
    lambda_smooth=0.01,   # ★ SMOOTHING: weight for action smoothing regularization
):
    ...
    for rnd in range(n_rounds):
        ...
        for stage_id in range(n_stages):
            ...
            # compute entropy from stored mean/std
            with torch.no_grad():
                mean, std = actors[stage_id].forward(obs_buffer[0])
            entropy = torch.distributions.Normal(mean, std).entropy().mean()

            # ---------- ★ SMOOTHING REGULARIZATION ----------
            # Use policy mean over the whole buffer to penalize large changes between consecutive timesteps
            if len(obs_buffer) > 1:
                obs_tensor = torch.stack(obs_buffer)  # (T, obs_dim)
                mean_all, _ = actors[stage_id].forward(obs_tensor)  # (T, 1)
                mean_all = mean_all.view(-1)  # (T,)
                smooth_loss = ((mean_all[1:] - mean_all[:-1]) ** 2).mean()
            else:
                smooth_loss = torch.tensor(0.0)

            # ---------- improved actor loss ----------
            # Original actor loss: -(logprobs * advantages).mean() - 0.01 * entropy
            # Add smoothing penalty: + lambda_smooth * smooth_loss
            actor_loss = (
                -(logprobs * advantages).mean()
                - 0.01 * entropy
        ...
        # (Optional) Print the average reward per round (the previous rewards represent the global reward across all stages).
        avg_reward = rewards.mean().item()
        print(
            f"Round {rnd+1}: approx last-episode avg reward = {avg_reward:.2f}, "
            f"smooth_loss ≈ {smooth_loss.item():.4f}"
        )


if __name__ == "__main__":
    # 单 stage 验证时，只改 n_stages 即可
    train_federated_actor_critic(
        n_stages=1,
        n_rounds=500,
        episodes_per_round=100,
        lambda_smooth=0.001,  # 先从 0.01 开始，如果还是 bang-bang 可以尝试 0.02 / 0.05
    )
