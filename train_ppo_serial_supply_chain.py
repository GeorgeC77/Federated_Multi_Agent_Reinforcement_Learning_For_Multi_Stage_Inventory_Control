...
def train_federated_actor_critic(
    ...
    for rnd in range(n_rounds):
        ...
        for stage_id in range(n_stages):
            ...
            rewards = torch.stack(reward_buffer)  # (T,)
            values = torch.stack(value_buffer)    # (T,)
            # Calculate simple Monte Carlo returns (can change it to GAE)
            returns = []
            G = 0.0
            ...
            # Actor loss (Simple PG, can change it to PPO clip)
        ...
        print(f"Round {rnd+1}: approx last-episode avg reward = {avg_reward:.2f}")


if __name__ == "__main__":
    train_federated_actor_critic(n_stages=2, n_rounds=100, episodes_per_round=100)
